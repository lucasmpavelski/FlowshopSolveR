...
int main(int argc, char* argv[]) {
    ...
    auto addProblemParam = [&prob_data, &addParam](std::string name, std::string def) {
...
//    parser.readFrom(test_input);
    ...
    if (!serve) {
        specs_file = parser.createParam(specs_file, "specs", "Algorithm specs", 'A')
                .value();
        ...
        for (auto ps : mh_specs) {
            ...
            param_value = parser.createParam(param_value, ps->name, "Parameter " + ps->name).value();
            values[ps->name] = ps->fromStrValue(param_value);
        }
        ...
        while (1) {
            ...
            bool done = net_parser.createParam(false, "done", "stop all").value();
            if (done) break;

            std::string mh_name = net_parser.createParam(string(), "mh", "metaheuristic name").value();
            std::transform(mh_name.begin(), mh_name.end(), mh_name.begin(), ::toupper);
            MHParamsSpecs mh_specs = MHParamsSpecsFactory::get(mh_name);

            MHParamsValues values(&mh_specs);
            for (auto ps : mh_specs) {
                std::string param_value = "0";
                param_value = net_parser.createParam(param_value, ps->name, "").value();
                values[ps->name] = ps->fromStrValue(param_value);
            }
