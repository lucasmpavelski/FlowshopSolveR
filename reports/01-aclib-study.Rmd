...
tunners <- c(unique(validation_dt$tunner), 'DEFAULT')
...
- Instance attributes:
  ...
  - Number of jobs: `r LARGE_INSTANCES_ATTRS$no_jobs`
  - Number of machines: `r LARGE_INSTANCES_ATTRS$no_machines`
  - Processing time distributions: `r LARGE_INSTANCES_ATTRS$dist`
  - Processing time correlations: `r LARGE_INSTANCES_ATTRS$corr`
...
ggplot(validation_results) +
  geom_violin(aes(x = tunner, fill = conf, y = rel_fitness)) +
  custom_theme +
  ggtitle('Violing plot of the relative performance of different tuners on all problems')
```
...
ggplot(validation_results) +
  ...
  scale_y_log10() +
  custom_theme +
  ggtitle('(log) Violing plot of the relative performance of different tuners on all problems')
```
...
ggplot(validation_results) +
  ...
  facet_grid(~corr) +
  custom_theme +
  ggtitle('(log) Violing plot of the relative performance of different tuners on all problems - by correlation')
```
...
ggplot(validation_results) +
  ...
  facet_grid(~type) +
  custom_theme +
  ggtitle('(log) Violing plot of the relative performance of different tuners on all problems - by constraint')
```
...
```{r, echo=F, warning=F, message=F}
rand_tunners_results <- validation_results %>%
  filter(tunner != 'DEFAULT', corr == 'rand')

friedmanTest(rand_tunners_results$rel_fitness,  rand_tunners_results$tunner, rand_tunners_results$instance)
```
