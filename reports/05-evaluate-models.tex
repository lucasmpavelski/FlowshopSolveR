...
\NormalTok{experiments <-}\StringTok{ }\KeywordTok{tribble}\NormalTok{(}
  ...
  \CommentTok{# 'decision_tree', 'ablation',}
  \CommentTok{# 'rand_forest', 'ablation',}
...
\NormalTok{  ) }\CommentTok{# %>% knitr::kable(format = 'latex')}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{groups-strat-5}{%
\section{Groups: strat {[}5{]}}\label{groups-strat-5}}

strat ml\_model q00 q25 q50 q75 q1 1 default none -8.49 0 0.435 1.63
28.5 2 global\_best none -9.91 0 0.449 1.41 14.3 3 instance-based
decision\_tree -8.91 0 0.175 0.887 13.3 4 instance-based rand\_forest
-8.49 0 0.155 0.817 24.0 5 instance-based-dependencies decision\_tree
-8.57 0 0.177 0.871 10.7 6 instance-based-dependencies rand\_forest
-8.37 -0.00834 0.0975 0.645 16.7 7 random none -9.32 1.09 4.52 16.0
2137.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(ggallin)}

\NormalTok{experiments }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{model_strat =} \KeywordTok{factor}\NormalTok{(}
    \KeywordTok{paste}\NormalTok{(ml_model, strat),}
    \DataTypeTok{levels =} \KeywordTok{c}\NormalTok{(}
      \StringTok{'none default'}\NormalTok{,}
      \StringTok{'none global_best'}\NormalTok{,}
      \StringTok{'decision_tree instance-based'}\NormalTok{,}
      \StringTok{'decision_tree instance-based-dependencies'}\NormalTok{,}
      \StringTok{'rand_forest instance-based'}\NormalTok{,}
      \StringTok{'rand_forest instance-based-dependencies'}
\NormalTok{    ),}
    \DataTypeTok{labels =} \KeywordTok{c}\NormalTok{(}
      \StringTok{'Standard NEH'}\NormalTok{,}
      \StringTok{'Global-best NEH'}\NormalTok{,}
      \StringTok{'DT'}\NormalTok{,}
      \StringTok{'DT+Dependencies'}\NormalTok{,}
      \StringTok{'RF'}\NormalTok{,}
      \StringTok{'RF+Dependencies'}
\NormalTok{    )}
\NormalTok{  )) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_violin}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ model_strat, }\DataTypeTok{fill =}\NormalTok{ model_strat, }\DataTypeTok{y =}\NormalTok{ rp)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_y_continuous}\NormalTok{(}\DataTypeTok{trans =}\NormalTok{ pseudolog10_trans) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_fill_viridis_d}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{theme_bw}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{axis.title =} \KeywordTok{element_blank}\NormalTok{(), }\DataTypeTok{axis.text.x =} \KeywordTok{element_blank}\NormalTok{()) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{fill =} \StringTok{"Strategy"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{05-evaluate-models_files/figure-latex/strategies-comparison-1.pdf}
...
\NormalTok{experiments }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(strat }\OperatorTok{!=}\StringTok{ 'random'}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# , ml_model != 'decision_tree'}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{model_strat =} \KeywordTok{factor}\NormalTok{(}
    \KeywordTok{paste}\NormalTok{(ml_model, strat),}
    \DataTypeTok{levels =} \KeywordTok{c}\NormalTok{(}
      \StringTok{'none default'}\NormalTok{,}
...
\includegraphics{05-evaluate-models_files/figure-latex/strategies-comparison-without-random-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{stat_data <-}\StringTok{ }\NormalTok{experiments }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
    \DataTypeTok{block =} \KeywordTok{paste}\NormalTok{(model, instance),}
    \DataTypeTok{group =} \KeywordTok{factor}\NormalTok{(}\KeywordTok{paste}\NormalTok{(ml_model, strat),}
    \DataTypeTok{levels =} \KeywordTok{c}\NormalTok{(}
      \StringTok{'rand_forest instance-based-dependencies'}\NormalTok{,}
      \StringTok{'rand_forest instance-based'}\NormalTok{,}
      \StringTok{'decision_tree instance-based-dependencies'}\NormalTok{,}
      \StringTok{'decision_tree instance-based'}\NormalTok{,}
      \StringTok{'none global_best'}\NormalTok{,}
      \StringTok{'none default'}\NormalTok{,}
      \StringTok{'none random'}
\NormalTok{    )),}
    \DataTypeTok{y =}\NormalTok{ rp}
\NormalTok{  ) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{select}\NormalTok{(block, group, y) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{arrange}\NormalTok{(group }\OperatorTok{!=}\StringTok{ 'rand_forest instance-based-dependencies'}\NormalTok{)}

\NormalTok{PMCMRplus}\OperatorTok{::}\KeywordTok{frdManyOneDemsarTest}\NormalTok{(stat_data}\OperatorTok{$}\NormalTok{y, stat_data}\OperatorTok{$}\NormalTok{group, stat_data}\OperatorTok{$}\NormalTok{block)}
\end{Highlighting}
\end{Shaded}
