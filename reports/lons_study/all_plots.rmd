...
```{r no_nodes_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
    metric == 'no_nodes'
  ) %>%
...
```{r clon_average_weight_of_self_loops_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
...
```{r avarege_out_degree_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
...
```{r clon_average_disparity_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
...
```{r clon_fitness_fitness_correlation_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
...
```{r neutral_no_groups_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
...
```{r compress_rate_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
...
```{r md_shiftDistance_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
...
```{r clon_average_weighted_clustering_coefficient_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  mutate(value = value) %>%
  filter(
...
```{r graph_clique_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  filter(
    metric == 'graph_clique',
  ) %>%
...
```{r average_fitness_bp, echo=F, fig.asp=0.3}
all_fla_tidy %>%
  mutate(value = value) %>%
  filter(
...
```{r neutral_rel_no_edges_bp, echo=F, fig.asp=0.3}
all_fla_tidy %>%
  mutate(value = value) %>%
  filter(
...
```{r clon_graph_assortativity_degree_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  mutate(value = value) %>%
  filter(
...
```{r graph_reciprocity_bp, echo=F, fig.asp=0.26}
all_fla_tidy %>%
  mutate(value = value) %>%
  filter(
