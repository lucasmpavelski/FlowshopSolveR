...
exp_folder <- here("reports", "aos", "data", "exp-03-adaptive_best_insertion")
...
perfs <- sample_performance(
  ...
  config = c(
    ...
    IG.AdaptiveBestInsertion.AOS.Strategy = "thompson_sampling",
    IG.AdaptiveBestInsertion.AOS.TS.Strategy = "static",
    IG.AdaptiveBestInsertion.AOS.WarmUp = "1000",
    IG.AdaptiveBestInsertion.AOS.WarmUp.Strategy = "random",
    IG.AdaptiveBestInsertion.AOS.Options  = "0_1_2_3",
    IG.AdaptiveBestInsertion.Replace = "yes",
    # IG.AdaptiveBestInsertion.NoArms = "fixed_3",
  ...
  solve_function = fsp_solver_performance,
  no_samples = 10,
  cache = file.path(perf_folder, paste0("ig-adapt-replace-perf.rds"))
)
...
perfs <- sample_performance(
  ...
  config = c(
    ...
    IG.AdaptiveBestInsertion.AOS.WarmUp.Strategy = "random",
    IG.AdaptiveBestInsertion.AOS.Options  = "0_1_2_3",
    IG.AdaptiveBestInsertion.Replace = "no",
    IG.AdaptiveBestInsertion.NoArms = "fixed_3",
  ...
  solve_function = function(...) {
    cat(list(...)$config)
    fsp_solver_performance_debug(...)
  },
  no_samples = 10,
  cache = file.path(perf_folder, paste0("ig-adapt-wo-replace-perf.rds"))
)
...
strategy_params <- tribble(
  ...
  mutate(
    name = paste0(ig_variant, '-adaptive-bi-', name),
    params = paste(base_config, params)
  ) %>%
  mutate(best_config = map2(name, params, run_irace))

# flowtime_test_problems <- test_problems %>%
#   keep(~.x@data['objective'] == 'FLOWTIME')
