...
write_aclib2_scenario <- function(aclib_scenario, scenarios_dir, ...) {
  scenario_dir <-
...
aclib_dir <- here('aclib2')
base_dir <- file.path(aclib_dir, 'fsp')
...
scenarios %>%
  pull(aclib_scenario) %>%
  writeLines(file.path(aclib_dir, 'experiments.txt'))

# write_experiments(scenarios)
#
# %>%
#   pmap(write_aclib2_scenarios)
