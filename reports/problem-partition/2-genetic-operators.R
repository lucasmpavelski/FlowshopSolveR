...
EXP_NAME <- "objective-medium-ga-hp"
...
final_configs <- population_to_configs(results$X, parameter_space)
...
plt <- tibble(
  ...
  mutate(fn = here(paste0("data/problem_partition/", EXP_NAME, "/eval_output_", generation, ".csv"))) %>%
  mutate(res = map(fn, read_csv)) %>%
  unnest(res) %>%
  select(FLOWTIME = `1`, MAKESPAN = `2`, generation) %>%
  ggplot() +
