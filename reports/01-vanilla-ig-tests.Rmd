...
```{r, results='asis'}
rewards %>%
  filter(instance ==  unique(rewards$instance)[4]) %>%
  mutate(gi = name %in% c("IG (d=2)", "IG (d=4)", "IG (d=8)")) %>%
  ggplot() +
  facet_wrap(~gi) +
  geom_histogram(aes(x = reward, fill = name)) +
  theme(axis.text = element_blank(), 
...
```{r}
rewards %>%
  filter(instance == "taillard_rand_500_20_01.dat") %>%
  mutate(g1 = name %in% c("IG (d=2)", "IG (d=4)", "IG (d=8)", "IG MAB")) %>%
  filter(g1) %>%
  ggplot() +
    facet_wrap(~instance, scales = "free") +
