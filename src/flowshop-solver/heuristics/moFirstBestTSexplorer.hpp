...
public:
     ...
     */
    moFirstBestTSexplorer(Neighborhood& _neighborhood,
		          ...
		          if (found) {
		              ...
		              while (neighborhood.cont(_solution) && !improveSol) {
		                  ...
		                  if ( (!tabuList.check(_solution, currentNeighbor) || aspiration(_solution, currentNeighbor)) && neighborComparator(selectedNeighbor, currentNeighbor)) {
		                      // set selectedNeighbor
		                      selectedNeighbor = currentNeighbor;
		                  	if ( solNeighborComparator(_solution, currentNeighbor) ) improveSol = true;
		                  }
		              }
									if(!improveSol) //no improving neighbor -> _solution=LO -> first improvement=desactivated / full exploration = activated
									 first=false;
		          }
		          else {
		              isAccept=false;
