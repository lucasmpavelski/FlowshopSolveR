...
void solveILS2(const int N,
  ...
  const int ls_nh_size =
      std::min(max_nh_size, min_nh_size + scale_ls * nh_interval);
  ...
  moFirstImprHC<Ngh> algo0(*neighborhood, fullEval, evalN, checkpoint,
                           *lscompNN, *lscompSN);  // FIHC
  moSimpleHC<Ngh> algo1(*neighborhood, fullEval, evalN, checkpoint, *lscompNN,
                        *lscompSN);  // BestHC
  moRandomBestHC<Ngh> algo2(*neighborhood, fullEval, evalN, checkpoint,
                            *lscompNN, *lscompSN);  // rndBestHC
  ...
  const double temperature =
      params.real("ILS.AcceptTemperature") * max_ct / (N * M * 10.0f);
  ...
  ****/
  ...
  OpPerturbDestConst<EOT> OpPerturb(
      fullEval, params.integer("ILS.PerturbDestructionSize"), *compSS);
