...
class plateauExplorerPerturb : public moPerturbation<Neighbor>, public moCountMoveMemory<Neighbor> {
...
public:
	...
	plateauExplorerPerturb(fspEval<EOT>& _fullEval, unsigned _MNS, eoMonOp<EOT>& _restart) : eval(_fullEval), MNS(_MNS), restart(_restart) {
		M=eval.getM();
		N=eval.getN();
		p=eval.getP();
...
//
		...
		if(nghNeutr_pos.size()==0){ // LO type 1
			...
			move2(_solution,nghNeutr_pos[k].first,nghNeutr_pos[k].second,nghNeutr_fit[k]);
			step++;
		...
		for(int k=0; k<RandJOB.size() && !found; ++k){
			...
			for(int j=1; j<M+1; ++j){
				...
				for(int i=1; i<N ; ++i){
					...
					e[i][j]=std::max(e[i][j-1],e[i-1][j])+p[j-1][solTMP[i-1]];
					q[N-i][M+1-j]=std::max(q[N-i][M+1-j+1],q[N-i+1][M+1-j])+p[M+1-j-1][solTMP[N-i-1]];
...
private:
	...
	std::vector< pair > nghNeutr_pos;  // first=position de départ; second=position d'arriver;
